...
export interface ClientOptions {
  /**
   * Defaults to process.env['POST_FOR_ME_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['POST_FOR_ME_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['POST_FOR_ME_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Post For Me API.
 */
export class PostForMe {
   ...
   * API Client for interfacing with the Post For Me API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['POST_FOR_ME_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['POST_FOR_ME_BASE_URL'] ?? https://api.postforme.dev] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('POST_FOR_ME_BASE_URL'),
    apiKey = readEnv('POST_FOR_ME_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.PostForMeError(
        "The POST_FOR_ME_API_KEY environment variable is missing or empty; either provide it, or instantiate the PostForMe client with an apiKey option, like new PostForMe({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.postforme.dev`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? PostForMe.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('POST_FOR_ME_LOG'), "process.env['POST_FOR_ME_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.postforme.dev';
  }
  ...
  static PostForMe = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static PostForMeError = Errors.PostForMeError;
...
export declare namespace PostForMe {
  ...
  export { Media as Media, type MediaCreateUploadURLResponse as MediaCreateUploadURLResponse };
  ...
  export {
    SocialPostResults as SocialPostResults,
